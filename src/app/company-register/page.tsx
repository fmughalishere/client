...
import {
  ...
  ArrowLeft, Check, Briefcase, Users, LayoutDashboard, Camera, Trash2, X, Scissors, Search, Map as MapIcon
} from "lucide-react";
...
export default function CompanyRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.password.length < 6) return toast.error("Password too short (min 6 chars)");
          ...
          <div className="fixed inset-0 z-[150] bg-white md:bg-black/60 md:backdrop-blur-sm flex items-center justify-center p-0 md:p-4">
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} className="bg-white w-full h-full md:h-auto md:max-w-2xl md:rounded-[2.5rem] flex flex-col overflow-hidden shadow-2xl">
              ...
              <div className="p-6 bg-gray-50 border-t shrink-0">
                <p className="text-[10px] text-center font-bold text-gray-400  mb-4">Click anywhere on the map to set your office pin</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-[2.5rem] bg-gray-50 border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden transition-all group-hover:border-[#00004d]">
                {logoPreview ? (
                  ...
                  <div className="text-center">
                    <Camera className="mx-auto text-gray-300 mb-1" size={28} />
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="relative">
                <Users className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300 z-10" size={18} />
                <select name="companySize" required value={formData.companySize} onChange={handleChange} className="w-full pl-12 pr-8 py-4 bg-gray-50 rounded-2xl outline-none border-2 border-transparent focus:border-[#00004d] font-bold text-[#00004d] text-sm appearance-none cursor-pointer">
                  <option value="">Company Size (Employees)</option>
                  {EMPLOYEES.map(size => <option key={size} value={size}>{size}</option>)}
                </select>
              </div>
              <div className="relative">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative">
                <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300 z-10" size={18} />
              ...
              <button type="button" onClick={() => setShowMapModal(true)} className="flex items-center justify-between px-6 py-4 bg-blue-50/50 text-[#00004d] rounded-2xl border-2 border-dashed border-blue-100 hover:border-[#00004d] transition-all">
                ...
                {formData.lat !== 31.5204 && <Check size={16} className="text-green-500" />}
              </button>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative">
                ...
                <input required type="text" name="contactPerson" placeholder="Full Name" value={formData.contactPerson} onChange={handleChange} className="w-full pl-12 pr-4 py-4 bg-gray-50 rounded-2xl outline-none border-2 border-transparent focus:border-[#00004d] font-bold text-[#00004d] text-sm" />
              </div>
              <div className="relative">
                <LayoutDashboard className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300" size={18} />
                <input required type="text" name="designation" placeholder="Designation (e.g. HR Manager)" value={formData.designation} onChange={handleChange} className="w-full pl-12 pr-4 py-4 bg-gray-50 rounded-2xl outline-none border-2 border-transparent focus:border-[#00004d] font-bold text-[#00004d] text-sm" />
              </div>
              <div className="relative">
          ...
          <div className="relative">
            ...
            <textarea required name="description" placeholder="Short description about your company..." rows={3} value={formData.description} onChange={handleChange} className="w-full pl-12 pr-4 py-4 bg-gray-50 rounded-2xl outline-none border-2 border-transparent focus:border-[#00004d] font-bold text-[#00004d] text-sm resize-none" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="relative">
          ...
          <div className={`flex items-start gap-4 p-5 rounded-3xl border-2 cursor-pointer transition-all ${agreed ? 'bg-[#00004d]/5 border-[#00004d]' : 'bg-gray-50 border-transparent'}`} onClick={() => setAgreed(!agreed)}>
            <div className={`mt-1 w-6 h-6 rounded-lg flex items-center justify-center shrink-0 ${agreed ? 'bg-[#00004d] shadow-lg shadow-blue-200' : 'border-2 border-gray-200 bg-white'}`}>
              {agreed && <Check size={14} className="text-white" strokeWidth={4} />}
            </div>
            <p className="text-[11px] font-bold text-gray-500  leading-relaxed">
              I agree to the <span className="text-[#00004d] underline">Terms of Service</span> and authorize account verification via the provided business email.
            </p>
          </div>
