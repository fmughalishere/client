  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col md:flex-row">
      <aside className="w-full md:w-72 bg-white border-r border-gray-100 p-8 flex flex-col gap-8">
        ...
        <div className="font-black text-[#000a31] text-xl px-2">
            {dashboardData?.user?.name || "Company"}
        </div>
        <nav className="space-y-2">
          {[
            ...
            <Link key={item.path} href={item.path} className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl font-bold transition-all ${pathname === item.path ? 'bg-[#000a31] text-white' : 'text-gray-400 hover:bg-slate-50'}`}>
      ...
      <main className="flex-1 p-6 md:p-12">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
          <div>
            <h1 className="text-3xl font-black text-[#000a31]">Employer Panel</h1>
            <p className="text-gray-400 font-bold">Managing {dashboardData?.user?.companyName || "your recruitment"}.</p>
          </div>
          <Link href="/dashboard/employer/post-job">
            <button className="bg-[#00d26a] text-white px-8 py-4 rounded-full font-black flex items-center gap-2 shadow-xl">
