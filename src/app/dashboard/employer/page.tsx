...
useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      const res = await fetch(`https://easyjobspk.onrender.com/api/dashboard/employer-stats?t=${new Date().getTime()}`, {
        headers: { 
  ...
  const stats = [
    { label: "Total Jobs", val: data?.totalJobsPosted || 0, icon: Briefcase, color: "text-red-600", bg: "bg-red-50" },
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-72 flex flex-col gap-4">
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex items-center lg:flex-col lg:items-center gap-4 text-center">
              <div className="w-14 h-14 bg-[#00004d] rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg shrink-0">
                {data?.user?.companyName?.substring(0, 1) || "E"}
              </div>
              <div className="text-left lg:text-center overflow-hidden w-full">
                <h3 className="font-black text-[#00004d] text-base lg:text-lg leading-tight truncate">
                  ...
                  >
                    {navItems.map((item) => (
                      <Link
                        ...
                        className={`flex items-center gap-3 px-6 py-4 font-bold text-sm border-b border-slate-50 last:border-0 ${
                          pathname === item.path ? 'bg-slate-50 text-[#00004d]' : 'text-slate-500'
                        }`}
                      >
          ...
          <div className="flex-1 space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-white/40 p-6 rounded-[2.5rem] border border-white/50">
              ...
              <Link href="/post-job" className="w-full sm:w-auto bg-[#5DBB63] hover:bg-[#4ea854] text-white px-8 py-4 rounded-2xl font-black flex items-center justify-center gap-3 shadow-lg shadow-green-900/20 active:scale-95 transition-all text-sm whitespace-nowrap">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              {stats.map((stat, i) => (
                <motion.div
                  ...
                  className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 group hover:border-[#00004d]/20 transition-all"
                >
                  ...
                  <div className="text-3xl font-black text-[#00004d] mb-1 tracking-tight">{stat.val}</div>
                  <div className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase truncate">{stat.label}</div>
                </motion.div>
              ))}
            </div>
            ...
            <section className="bg-white p-6 md:p-10 rounded-[3rem] border border-slate-100 shadow-sm">
              <div className="flex justify-between items-center mb-8 px-2">
                ...
                <Link href="/dashboard/employer/applicants" className="p-2 bg-slate-50 rounded-xl hover:bg-[#00004d] hover:text-white transition-all text-slate-400">
                  ...
                  data?.recentApplicants?.map((app: any) => (
                    <div key={app._id} className="group flex items-center justify-between p-4 md:p-6 rounded-[2rem] border border-slate-50 hover:bg-slate-50/50 hover:border-[#00004d]/10 transition-all">
                      <div className="flex items-center gap-4 min-w-0">
                        <div className="w-12 h-12 bg-[#e6e8e8] rounded-2xl flex items-center justify-center text-[#00004d] font-black text-lg group-hover:bg-[#00004d] group-hover:text-white transition-all shadow-sm shrink-0">
                          {app.fullName?.substring(0, 1) || "A"}
                        </div>
                        <div className="min-w-0">
                          <h4 className="font-black text-[#00004d] text-base leading-tight truncate">{app.fullName}</h4>
                          <p className="text-xs font-bold text-slate-400 mt-1 truncate">
                            Role: <span className="text-[#5DBB63]">{app.job?.title || "Not Specified"}</span>
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4 shrink-0">
                        <span className={`hidden sm:block text-[10px] font-black px-4 py-2 border rounded-full uppercase tracking-wider shadow-sm ${getStatusStyles(app.status)}`}>
                          {app.status || "Pending"}
                        </span>
