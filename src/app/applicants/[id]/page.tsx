    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 50 }} className="relative w-full max-w-[380px] bg-white rounded-[35px] p-8 text-center shadow-2xl">
            <div className="flex justify-center mb-6"><div className="bg-green-100 p-4 rounded-full"><CheckCircle2 size={50} className="text-green-500" /></div></div>
            <h3 className="text-2xl font-bold text-[#00004d] mb-3">Offer Sent Successfully!</h3>
            <p className="text-gray-600 mb-6 leading-relaxed">Your job offer has been delivered to the candidate. They will review your proposal and get in touch with you directly.<br /><span className="text-[12px] text-gray-400 mt-2 block">آپ کی آفر امیدوار کو بھیج دی گئی ہے۔ وہ آپ سے جلد رابطہ کریں گے۔</span></p>
            <button onClick={onAction} className="w-full bg-[#00004d] text-white py-4 rounded-2xl font-bold text-sm active:scale-95 transition-transform shadow-lg shadow-blue-900/20">Back to Applicants</button>
          </motion.div>
        </div>
      )}
    </AnimatePresence>
...
export default function ApplicantDetail() {
  ...
  return (
    <div className="min-h-screen bg-[#e6e8e8] p-4 md:p-10 pb-24 font-sans">
      <AnimatePresence>
        {isCropping && (
          <div className="fixed inset-0 z-[120] flex flex-col items-center justify-center bg-black/90 p-4">
            ...
            <div className="mt-6 w-full max-w-[400px] space-y-4 px-4">
              <input type="range" value={zoom} min={1} max={3} step={0.1} onChange={(e) => setZoom(Number(e.target.value))} className="w-full h-1.5 accent-white" />
              <div className="flex gap-3">
                <button onClick={() => setIsCropping(false)} className="flex-1 py-3.5 bg-white/10 text-white rounded-xl font-bold border border-white/20">Cancel</button>
                <button onClick={saveCroppedLogo} className="flex-1 py-3.5 bg-white text-[#00004d] rounded-xl font-bold">Save Logo</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="bg-white rounded-[35px] shadow-lg flex items-stretch overflow-hidden min-h-[140px]">
          <div className={`w-32 md:w-48 shrink-0 flex items-center justify-center ${applicant.image === "male" ? "bg-[#00004d]" : applicant.image === "female" ? "bg-[#5DBB63]" : "bg-gray-100"}`}>
            {applicant.image === "male" ? (
              <img src={MALE_ICON} className="w-[65%] h-[65%] object-contain" style={whiteFilter} alt="M" />
            ) : applicant.image === "female" ? (
              <img src={FEMALE_ICON} className="w-[65%] h-[65%] object-contain" style={whiteFilter} alt="F" />
            ) : applicant.image ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-8 rounded-3xl shadow-sm space-y-5">
            <h2 className="text-lg font-bold text-[#00004d] tracking-wider flex items-center gap-2 border-b pb-3"><User size={18} /> Basic Info</h2>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm space-y-5">
            <h2 className="text-lg font-bold text-[#00004d] tracking-wider flex items-center gap-2 border-b pb-3"><GraduationCap size={18} /> Education</h2>
            <p className="flex items-center gap-2 text-sm font-bold text-gray-600"><GraduationCap size={16} className="text-gray-400" /> {applicant.education}</p>
          </div>
        </div>

        <div className="bg-white p-8 rounded-3xl shadow-sm space-y-5">
          <h2 className="text-lg font-bold text-[#00004d] tracking-wider flex items-center gap-2 border-b pb-3"><Briefcase size={18} /> Work Experience</h2>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm space-y-5 text-center">
          <h2 className="text-lg font-bold text-[#00004d] tracking-wider border-b pb-3 inline-block">💰 Salary Expectation</h2>
          <p className="text-xl font-black text-[#5DBB63] mt-4">{applicant.salaryDemand || "Not mentioned"}</p>
        </div>

        <div className="mt-8 flex justify-center pb-2">
          <button onClick={() => setIsModalOpen(true)} disabled={applicant.status === "Offered"} className={`w-50 h-10 px-8 rounded-[9px] font-bold text-sm shadow-2xl flex items-center justify-center gap-2 active:scale-95 transition-all tracking-[0.2em] ${applicant.status === "Offered" ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#5DBB63] text-white'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
          <div className="bg-white w-full max-w-lg rounded-[45px] overflow-hidden shadow-2xl relative">
            ...
            <form onSubmit={handleFormSubmit} className="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
              ...
              <input required name="companyName" placeholder="Company Name" onChange={handleInputChange} className="w-full p-4 bg-gray-50 rounded-xl outline-none border font-bold text-xs" />
              <input required name="address" placeholder="Office Address" onChange={handleInputChange} className="w-full p-4 bg-gray-50 rounded-xl outline-none border font-bold text-xs" />
              <input required name="cityName" placeholder="City" onChange={handleInputChange} className="w-full p-4 bg-gray-50 rounded-xl outline-none border font-bold text-xs" />
              <input required name="employerName" placeholder="Manager Name" onChange={handleInputChange} className="w-full p-4 bg-gray-50 rounded-xl outline-none border font-bold text-xs" />
              <input required name="designation" placeholder="Job Title" onChange={handleInputChange} className="w-full p-4 bg-gray-50 rounded-xl outline-none border font-bold text-xs" />
              <input type="email" required name="email" placeholder="Official Email" onChange={handleInputChange} className="w-full p-4 bg-gray-50 rounded-xl outline-none border font-bold text-xs" />
              <input name="salaryRange" placeholder="Salary Range" onChange={handleInputChange} className="w-full p-4 bg-gray-50 rounded-xl outline-none border font-bold text-xs" />
              <div className="relative">
                <FaWhatsapp className="absolute left-3 top-4 text-green-500" size={16} />
                <input required name="whatsapp" placeholder="WhatsApp Number" onChange={handleInputChange} className="w-full p-4 pl-10 bg-gray-50 rounded-xl outline-none border font-bold text-xs" />
              </div>
