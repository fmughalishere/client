          ...
          <div className="bg-white w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-[3rem] shadow-2xl relative">
            ...
            <form onSubmit={handleFormSubmit} className="p-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 ml-2 tracking-widest">City</label>
                <input required value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border-none font-bold" /></div>
                <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 ml-2 tracking-widest">WhatsApp</label>
                <input required value={formData.whatsapp} onChange={e => setFormData({...formData, whatsapp: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl border-none font-bold" /></div>
              </div>
              <button type="submit" disabled={isApplying} className="w-full py-5 bg-[#5DBB63] text-white rounded-[2rem] font-black shadow-xl flex items-center justify-center gap-3">
                {isApplying ? <Loader2 className="animate-spin" /> : <Send size={20} />} Complete & Apply
              </button>
            </form>
          </div>
        </div>
